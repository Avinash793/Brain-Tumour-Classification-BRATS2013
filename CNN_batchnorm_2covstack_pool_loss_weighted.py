...
def CNN_Arch(image_size):
    ...
    model = Sequential()
    model.add(Conv2D(64, (3,3), strides=(1,1), activation='relu', input_shape=image_size))
...
filepath = "./model_cnn_Avi/cnn_batchnorm_2convstack_pool_wei.hdf5"
checkpoint = ModelCheckpoint(filepath, monitor='val_acc', verbose=1, save_best_only=True, mode='max')
model.fit(x_data,y_data, epochs=EPOCHS, batch_size=BATCH_SIZE, verbose=1,validation_data=(x_test,y_test),class_weight=class_weight)

model.save_weights("CNN_batchnorm_2convstack_pool_wei.h5")